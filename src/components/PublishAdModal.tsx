...
export const PublishAdModal = ({ games }: PublishAdModalProps) => {
  ...
  const handlePublishAd = useCallback(async (e: FormEvent) => {
    ...
    try {
      ...
      toast.error("And error occurred during the ad publishing");
    } finally {
      setIsLoading(false);
    }
  }, [weekDays]);
    ...
    <Dialog.Portal>
      ...
      <Dialog.Content className="fixed bg-[#2a2634] py-8 px-10 text-white top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 rounded-lg w-[550px] shadow-lg shadow-black/25">
        ...
        {isLoading ? (
          ...
          <form onSubmit={handlePublishAd} className="mt-8 flex flex-col gap-4">
